...
# svhn dataset (images and labels)
svhn_train_dataset = torchvision.datasets.SVHN(root='./data', split='train', download=True, transform=transform)
svhn_test_dataset = torchvision.datasets.SVHN(root='./data', split='test', download=True, transform=transform)


# classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')

# Data loader (input pipeline)
train_loader = torch.utils.data.DataLoader(dataset=svhn_train_dataset, batch_size=batch_size, shuffle=True)
valid_loader = torch.utils.data.DataLoader(dataset=svhn_test_dataset, batch_size=batch_size, shuffle=False)
...
for epoch in range(epochs):
    ...
    np.save(os.path.join(save_model_path, 'y_svhn_train_epoch{}.npy'.format(epoch + 1)), y_train)
    np.save(os.path.join(save_model_path, 'z_svhn_train_epoch{}.npy'.format(epoch + 1)), z_train)
