...
# save model
save_model_path = './results_emnist'
...
emnist_train_dataset = torchvision.datasets.EMNIST(root='./data', split = 'balanced', train=True, download=True, transform=transform)
emnist_test_dataset = torchvision.datasets.EMNIST(root='./data', split = 'balanced', train=False, download=True, transform=transform)


# classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')

# Data loader (input pipeline)
train_loader = torch.utils.data.DataLoader(dataset=emnist_train_dataset, batch_size=batch_size, shuffle=True)
valid_loader = torch.utils.data.DataLoader(dataset=emnist_test_dataset, batch_size=batch_size, shuffle=False)
...
for epoch in range(epochs):
    ...
    np.save(os.path.join(save_model_path, 'y_emnist_train_epoch{}.npy'.format(epoch + 1)), y_train)
    np.save(os.path.join(save_model_path, 'z_emnist_train_epoch{}.npy'.format(epoch + 1)), z_train)
